...
while my_sample["next"] !=None:
    ...
    if sensor_modality in ['lidar', 'radar']:
        ...
        if sensor== "radarlidar":
            ...
            key_point=np.zeros((2,points_radar.shape[1]))
            key_point[0,:]=0
            key_point[1,:]=35

            dists_radar = np.sqrt(np.sum((pc_times_radar[:2, :]-key_point) ** 2, axis=0))
            colors = np.minimum(1, dists_radar / 20 / np.sqrt(2))
            for i in range(len(colors)):
                if colors[i]< 0.1:
                    colors[i]=colors[i]+random.uniform(-0.05, 0.05)
                elif colors[i]< 0.3:
                    colors[i]=colors[i]+random.uniform(-0.25, 0.25)
                elif colors[i]< 0.6:
