...
while my_sample["next"] !=None:
    ...
    if sensor_modality in ['lidar', 'radar']:
        ...
        if sensor== "radarlidar":
            ...
            point_scale = 0.0005
            ax.scatter(points_lidar[0, :], points_lidar[1, :],alpha=0.5 , c="black", s=point_scale)
            ...
            scatter=ax.scatter(points_radar_ghost[0, :], points_radar_ghost[1, :], c=dists_radar_ghost, cmap="winter",vmin=0, vmax=60,s=point_scale)
        ...
        if sensor == 'radar':
            ...
            deltas_vel = np.clip(deltas_vel, -max_delta, max_delta)  # Arbitrary clipping
            colors_rgba = scatter.to_rgba(colors)
            ghost_points_radar = points_radar[:, ghost_mask]
            np.save('/home/leichen/OpenPCDet/data/nuscenes/v1.0-mini/test.npy',pc_times_radar[:3, ghost_mask])
            ghost_deltas_vel = deltas_vel[:, ghost_mask]

            for i in range(ghost_points_radar.shape[1]):
                ax.arrow(ghost_points_radar[0, i], ghost_points_radar[1, i], ghost_deltas_vel[0, i], ghost_deltas_vel[1, i], color=colors_rgba[i])
