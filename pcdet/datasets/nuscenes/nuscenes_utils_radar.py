...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  4 11:38:09 2021
...
def get_available_scenes(nusc):
    ...
    for scene in nusc.scene:
        ...
        #  here only judge if the radar data exist, no merge
...
# here is an issue that get_sample_data can only get one sensor
def get_sample_data(nusc, sample_data_token, selected_anntokens=None):
    ...
    box_list = []
    for box in boxes:
        # ToDo 2: how to make the box bigger? 
...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    # maybe ref_chan should be LIDAR_TOP
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        info = {
            # ToDo 3 no LiDAR path, but radar path
        ...
        while len(sweeps) < max_sweeps - 1:
            if curr_sd_rec['prev'] == '':
                if len(sweeps) == 0:
                    sweep = {
                        # ToDo 4 no LiDAR path, but radar path
...
#ToDo 7 change to radar_box_to_global
...
''' we don't do detection in this version
