...
class NuScenesDataset(DatasetTemplate):
    ...
    def get_radar_with_sweeps(self, index, max_sweeps=1):
        ...
        print("pc_F_times",pc_F_times)
        print("pc_F_times_shape",pc_F_times.shape)
        ...
        print("pc_F_L_times",pc_F_L_times)
        print("pc_F_L_times_shape",pc_F_L_times.shape)
        ...
        print("pc_F_R_times",pc_F_R_times)
        print("pc_F_R_times_shape",pc_F_R_times.shape)
        ...
        print("pc_times",pc_times)
        print("pc_times_shape",pc_times.shape)
        # here the dimen is wrong!
        points=pc_times[:5, :].T
    ...
    def __getitem__(self, index):
        ...
        points = self.get_radar_with_sweeps(index, max_sweeps=self.dataset_cfg.MAX_SWEEPS)
    ...
    def create_groundtruth_database(self, used_classes=None, max_sweeps=10):
        ...
        database_save_path = self.root_path / f'gt_database_{max_sweeps}sweeps_radar'
        db_info_save_path = self.root_path / f'nuscenes_dbinfos_{max_sweeps}sweeps_radar.pkl'
        ...
        for idx in tqdm(range(len(self.infos))):
            ...
            info = self.infos[idx]
            # change to radar 
            points = self.get_radar_with_sweeps(idx, max_sweeps=max_sweeps)
            ...
            print("box_idxs_of_pts",box_idxs_of_pts)
            '''
            ...
            '''
            # 4.2.2021 -> try to merge all radar, make the boxes larger
            ...
            if (used_classes is None) :
                db_path = str(filepath.relative_to(self.root_path))  # gt_database/xxxxx.bin
                db_info = {'name': gt_names[:], 'path': db_path, 'image_idx': sample_idx, 
                           'box3d_lidar': gt_boxes[:], 'num_points_in_gt': gt_points.shape[0]}
                #???????????
                if sample_idx in all_db_infos:
                    all_db_infos[sample_idx].append(db_info)
                else:
                    all_db_infos[sample_idx] = [db_info]
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10):
    ...
    from . import nuscenes_utils_radar
    ...
    available_scenes = nuscenes_utils_radar.get_available_scenes(nusc)
    ...
    train_nusc_infos, val_nusc_infos = nuscenes_utils_radar.fill_trainval_infos(
